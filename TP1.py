...
def pasar_a_yiq():
    ...
    if not imagen_original:
        label_info.config(text="Primero seleccione una imagen", fg="red")
        return
...
def mostrar_canales():
    ...
    if imagen_yiq is None:
        label_info.config(text="Primero convierta la imagen a YIQ", fg="red")
        return
...
def modificar_y_i():
    print("Falta esta funcion")

def guardar():
    if imagen_mostrada is None:
        label_info.config(text="No hay imagen procesada para guardar", fg="red")
        return
